...
module.exports = function(gulp) {
  ...
  gulp.task('init wrapper', 'Grabs each component from GitHub', initComponents);
  gulp.task('create wrapper', 'Updates files in each repo', createComponents);

  /* Publish */
  gulp.task('update distributions', 'Commits component updates from create to GitHub', updateDistributions);
  gulp.task('update wrapper', 'Commits component updates from create to GitHub', updateComponents);

  /* Tasks */
  gulp.task('release', 'Stages changes in GitHub repos for all distributions', release);
  gulp.task('publish', 'Publishes all releases (wrapper, package)', publish);
